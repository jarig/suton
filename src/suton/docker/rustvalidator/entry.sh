...
# /ton-node-work is mount point. Then we create subfolder, so in case of env-switch container shouldn't be re-created
...
if [[ ! -f "$TON_CONSOLE_KEYS_ROOT/client" || ! -f "$TON_NODE_CONFIGS_DIR/config.json" || ! -d "$TON_NODE_ENV_DB_DIR/db" ]]; then
  ...
  if [[ -f "$TON_NODE_ENV_CONFIGS_DIR/config.json" ]]
  then
    echo "Re-using existing configuration from $TON_NODE_ENV_CONFIGS_DIR"
    # use existing config
    sudo mkdir -p $TON_NODE_CONFIGS_DIR
    sudo cp -f "$TON_NODE_ENV_CONFIGS_DIR/config.json" "$TON_NODE_CONFIGS_DIR/config.json"
    sudo chown -R ton:ton "$TON_NODE_CONFIGS_DIR"
  else
    ...
    jq ".restore_db = false" "${TMP_DIR}/config.json.tmp" >"${TMP_DIR}/config.json.tmp2"
    cp "${TMP_DIR}/config.json.tmp2" "${TON_NODE_CONFIGS_DIR}/config.json"
